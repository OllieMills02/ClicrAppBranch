...
import { ArrowLeft, Settings2, Plus, Minus, ScanFace, CheckCircle2, XCircle, ArrowUpCircle, ArrowDownCircle, Trash2, Layout, Link2, Unlink, ChevronDown, Check } from 'lucide-react';
...
export default function ClicrCounterPage() {
    ...
    const handleReset = async () => {
        ...
        try {
            ...
            await fetch('/api/sync', {
                ...
                body: JSON.stringify({ action: 'RESET_COUNTS', venue_id: 'ven_001' }), // Explicit venue_id
                cache: 'no-store'
            });
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-100px)] -mt-4 bg-black relative" onClick={() => inputRef.current?.focus()}>
            ...
            {/* Top Bar */}
            <div className="flex items-center justify-between mb-2 z-30 relative">
                    ...
                    <AnimatePresence>
                        {showLayoutMenu && (
                            <motion.div
                                ...
                                exit={{ opacity: 0, y: -10 }}
                                className="absolute right-0 top-full mt-2 w-56 bg-[#0f1218] border border-slate-800 rounded-xl shadow-2xl overflow-hidden z-50 p-2"
                            >
                                <div className="text-xs font-bold text-slate-500 uppercase tracking-widest px-3 py-2">Screen Layout</div>
                                <button
                                    onClick={() => { setLayoutMode('SINGLE'); setShowLayoutMenu(false); }}
                                    className={cn("w-full flex items-center justify-between px-3 py-3 rounded-lg text-sm font-bold transition-colors", layoutMode === 'SINGLE' ? "bg-primary/10 text-primary" : "text-white hover:bg-slate-800")}
                                >
                                    <span>Single View</span>
                                    {layoutMode === 'SINGLE' && <Check className="w-4 h-4" />}
                                </button>
                                <button
                                    onClick={() => activateSplit('INDEPENDENT')}
                                    className={cn("w-full flex items-center justify-between px-3 py-3 rounded-lg text-sm font-bold transition-colors", (layoutMode === 'SPLIT' && splitConfig.mode === 'INDEPENDENT') ? "bg-primary/10 text-primary" : "text-white hover:bg-slate-800")}
                                >
                                    <span>Split (2 Areas)</span>
                                    {(layoutMode === 'SPLIT' && splitConfig.mode === 'INDEPENDENT') && <Check className="w-4 h-4" />}
                                </button>
                                <button
                                    onClick={() => activateSplit('LINKED')}
                                    className={cn("w-full flex items-center justify-between px-3 py-3 rounded-lg text-sm font-bold transition-colors", (layoutMode === 'SPLIT' && splitConfig.mode === 'LINKED') ? "bg-primary/10 text-primary" : "text-white hover:bg-slate-800")}
                                >
                                    <span>Linked (Entry/Exit)</span>
                                    {(layoutMode === 'SPLIT' && splitConfig.mode === 'LINKED') && <Check className="w-4 h-4" />}
                                </button>
                            </motion.div>
                        )}
                    </AnimatePresence>
            ...
            {layoutMode === 'SINGLE' && (
                <div className="flex-1 flex flex-col gap-4 relative animate-[fade-in_0.3s_ease-out]">
            ...
            {layoutMode === 'SPLIT' && (
                <div className="flex-1 flex flex-col gap-1 p-2 animate-[fade-in_0.3s_ease-out]">
                    {/* Top Partition (Primary) */}
                    <SplitCounterPart
                        clicrId={clicr.id}
                        label={splitConfig.primaryLabel || clicr.name}
                        color="blue"
                        mode={splitConfig.mode}
                        role="PRIMARY" // Linked: Entries
                        onTap={(delta) => {
                            // If linked, simpler logic: Top is ENTRY (+1)
                            if (splitConfig.mode === 'LINKED') {
                                recordEvent({ venue_id: 'ven_001', area_id: clicr.area_id, clicr_id: clicr.id, delta: 1, flow_type: 'IN', event_type: 'TAP', idempotency_key: Math.random().toString() });
                            } else {
                                // Independent: Standard +/-
                                recordEvent({ venue_id: 'ven_001', area_id: clicr.area_id, clicr_id: clicr.id, delta: delta, flow_type: delta > 0 ? 'IN' : 'OUT', event_type: 'TAP', idempotency_key: Math.random().toString() });
                            }
                        }}
                        currentCount={clicr.current_count} // Linked uses same count
                    />

                    {/* Bottom Partition (Secondary) */}
                    <SplitCounterPart
                        clicrId={splitConfig.secondaryClicrId}
                        label={splitConfig.secondaryLabel || 'Secondary'}
                        color={splitConfig.mode === 'LINKED' ? 'red' : 'pink'} // Linked = Red (Exit), Indep = Pink
                        mode={splitConfig.mode}
                        role="SECONDARY" // Linked: Exits
                        onTap={(delta) => {
                            if (splitConfig.mode === 'LINKED') {
                                // Bottom is EXIT (Effective -1 to count, but recorded as OUT flow)
                                recordEvent({ venue_id: 'ven_001', area_id: clicr.area_id, clicr_id: clicr.id, delta: -1, flow_type: 'OUT', event_type: 'TAP', idempotency_key: Math.random().toString() });
                            } else {
                                if (splitConfig.secondaryClicrId) {
                                    const secClicr = clicrs.find(c => c.id === splitConfig.secondaryClicrId);
                                    if (secClicr) {
                                        recordEvent({ venue_id: 'ven_001', area_id: secClicr.area_id, clicr_id: secClicr.id, delta: delta, flow_type: delta > 0 ? 'IN' : 'OUT', event_type: 'TAP', idempotency_key: Math.random().toString() });
                                    }
                                }
                            }
                        }}
                        currentCount={splitConfig.mode === 'LINKED' ? clicr.current_count : (clicrs.find(c => c.id === splitConfig.secondaryClicrId)?.current_count || 0)}
                    />
                </div>
            )}
